...
app.post('/cocktails/', async (req, res) => {//endpoint to post a new cocktail to dbase
  try {
    ...
    const newType = req.body.alcohol_type;
    ...
    const data = await client.query(`
      ...
      [newName, newStrength, newType, newHotDrink, newOwnerId]);//add these to the cocktail array
...
app.put('/cocktails/:id', async (req, res) => {//new put route to update existing cocktail table data...
  try {
    ...
    const data = await client.query(`
      ...
      RETURNING *;
    `,
      [newName, newStrength, newType, newHotDrink, newOwnerId, req.params.id]);//add these to the cocktail array (puts to the front?)
